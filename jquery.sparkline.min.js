...
target.drawRect(x,y,options.barWidth-1,height-1,color,color);}}else{this.innerHTML='';}};$.fn.sparkline.tristate=function(values,options,width,height){values=$.map(values,Number);var options=$.extend({barWidth:4,barSpacing:1,posBarColor:'#6f6',negBarColor:'#f44',zeroBarColor:'#999',colorMap:{},equalHeights:'no'},options);var width=(values.length*options.barWidth)+((values.length-1)*options.barSpacing);var target=$(this).simpledraw(width,height);if(target){var canvas_width=target.pixel_width;var canvas_height=target.pixel_height;var bar_height=options.equalHeights=='no'?Math.round(canvas_height/2):bar_height=Math.round(canvas_height/3);for(var i=0;i<values.length;i++){var x=i*(options.barWidth+options.barSpacing);if(values[i]<0){var y=options.equalHeights=='no'?(bar_height+1):((bar_height*2)-2);var height=bar_height-1;var color=options.negBarColor;}else if(values[i]>0){var y=0;var height=bar_height-1;var color=options.posBarColor;}else{var y=bar_height-1;var height=options.equalHeights=='no'?2:(bar_height-1);var color=options.zeroBarColor;}
